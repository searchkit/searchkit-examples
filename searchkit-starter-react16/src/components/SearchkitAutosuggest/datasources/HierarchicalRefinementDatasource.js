import { 
    NestedFacetAccessor, RangeQuery, TopHitsMetric, BoolMust,
    FilterBucket, NestedQuery, NestedBucket, TermsBucket, LevelState
} from "searchkit"
...
export class HierarchicalRefinementDatasource {
    ...
    configure(searchkit) {
        ...
        if (this.options.accessorId) {
            ...
            let { id, field, operator, startLevel, title } = this.options
            this.originalAccessor = new NestedFacetAccessor(id, {
                id, field, operator, startLevel, title
            })
    ...
    search(query, queryString) {
        ...
        const regexpQuery = {
            regexp: {
                [field + ".value"]: Utils.createRegexQuery(queryString)
            }
        }
